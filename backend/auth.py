...
async def get_current_farmer(credentials: HTTPAuthorizationCredentials = Depends(security), db: Session = Depends(get_db)):
    ...
    farmer = get_farmer_by_email(db, email=token_data.email)
    if farmer is None:
        raise credentials_exception
    return farmer
